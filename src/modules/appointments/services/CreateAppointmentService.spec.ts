...
describe('CreateAppointment', () => {
  ...
  it('should not be able to create appointments with the user being the same as the provider', async () => {
    jest.spyOn(Date, 'now').mockImplementationOnce(() => {
      return new Date(2020, 4, 10, 12).getTime();
    });
  ...
  it('should not be able to create appointments outside the working hours', async () => {
    jest.spyOn(Date, 'now').mockImplementationOnce(() => {
      return new Date(2020, 4, 10, 12).getTime();
    });
